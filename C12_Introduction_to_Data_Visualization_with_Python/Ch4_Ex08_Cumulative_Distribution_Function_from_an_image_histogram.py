...
# A histogram of a continuous random variable is sometimes called a Probability Distribution Function (or PDF). The
# area under a PDF (a definite integral) is called a Cumulative Distribution Function (or CDF). The CDF quantifies
# the probability of observing certain pixel intensities.

# Your task here is to plot the PDF and CDF of pixel intensities from a grayscale image. You will use the grayscale
# image of Hawkes Bay, New Zealand (originally by Phillip Capper, modified by User:Konstable, via Wikimedia Commons,
# CC BY 2.0). This time, the 2D array image will be pre-loaded and pre-flattened into the 1D array pixels for you.
...
# - The command plt.twinx() allows two plots to be overlayed sharing the x-axis but with different scales on the
# y-axis.
