...
# For grayscale images, various image processing algorithms use an image histogram. Recall that an image is a
# two-dimensional array of numerical intensities. An image histogram, then, is computed by counting the occurences
# of distinct pixel intensities over all the pixels in the image.

# For this exercise, you will load an unequalized low contrast image of Hawkes Bay, New Zealand (originally by Phillip
# Capper, modified by User:Konstable, via Wikimedia Commons, CC BY 2.0). You will plot the image and use the pixel
# intensity values to plot a normalized histogram of pixel intensities.
