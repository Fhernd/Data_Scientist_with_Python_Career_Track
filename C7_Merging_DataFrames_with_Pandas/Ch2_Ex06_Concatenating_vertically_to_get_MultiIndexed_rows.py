...
# pd.concat(), which generates a hierarchical index with the labels from keys as the outermost index label. So you
# don't have to rename the columns of each DataFrame as you load it. Instead, only the Index column needs to be
# specified.

# Here, you'll continue working with DataFrames compiled from The Guardian's Olympic medal dataset. Once again, pandas
# has been imported as pd and two lists have been pre-loaded: An empty list called medals, and medal_types, which
