...
# for each row in the left DataFrame, only rows from the right DataFrame whose 'on' column values are less than the
...
# Here, you'll merge monthly oil prices (US dollars) into a full automobile fuel efficiency dataset. The oil and
# automobile DataFrames have been pre-loaded as oil and auto. The first 5 rows of each have been printed in the IPython
# Shell for you to explore.
...
# You'll then inspect the merged DataFrame, resample by year and compute the mean 'Price' and 'mpg'. You should be able
# to see a trend in these two columns, that you can confirm by computing the Pearson correlation between resampled
# 'Price' and 'mpg'.
...
# - Resample merged using 'A' (annual frequency), and on='Date'. Select [['mpg','Price']] and aggregate the mean. Store
# the result as yearly.
